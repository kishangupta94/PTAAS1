...
class BurpExtender(IBurpExtender, IHttpListener, IScannerListener):
	...
	self._callbacks.setExtensionName("PTAAS")
	...
	print "Initiating PTAAS Against: ", str(self.url)
	...
	file_name = 'PTAAS_'+self.scheme+'_'+self.fqdn+'_'+str(self.port)+'.'+format.lower()
	...
	if len(cli) < 0:
		...
		print "PTAAS is now loaded."
		print "If PTAAS was loaded through the BApp store then you can run in headless mode simply adding the `-Djava.awt.headless=true` flag from within your shell. Note: If burp doesn't close at the conclusion of a scan then disable Automatic Backup on Exit."
